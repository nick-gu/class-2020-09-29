...
```{r, echo = FALSE}
read_enrollment <- function(filepath, n = 3){
   x <- read_excel(filepath, skip = n) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select( -instructor_full_name, -course_section_code, -(grad:total)) %>%
  rename_with(~ str_replace(.x, "course_", "")) %>% 
...
total <- bind_rows(d_2019_take_2, d_2018, d_2017, d_2016, d_2015, .id = "year") %>%
  arrange(id)
...
total %>% ggplot(mapping = aes(x = year, y = u_grad)) +
  geom_col(position = "dodge") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + facet_wrap(~department)
